...
class Actor_pc(nn.Module):
    def __init__(self,normal_channel=True):
        super(Actor_pc, self).__init__()
        
        # in_channel = 3 if normal_channel else 0
        self.normal_channel = normal_channel
        self.sa1 = PointNetSetAbstraction(npoint=512, radius=0.2, nsample=32, in_channel=6, mlp=[64,128],
                                          group_all=False)
        ...
        self.sa3 = PointNetSetAbstraction(npoint=None, radius=None, nsample=None, in_channel=256 + 3,
                                          mlp=[256,256,128], group_all=True)
    ...
    def forward(self, xyz, state):
        ...
        x = torch.cat((x, state), 1).contiguous()
...
class ReplayBuffer:
    def __init__(self, mode, w, h, action_dim, info_dim, size):
        ...
        if mode == 'rgbd' or mode=='de':
            c = 6 if mode == 'de' else 4
            self.sta1_buf = np.zeros([size, c, h, w], dtype=np.uint8)
            self.sta2_buf = np.zeros([size, c, h, w], dtype=np.uint8)
        elif mode =='pc':
            npoints=2048
            self.sta1_buf = np.zeros([size, 6,npoints], dtype=np.float32)
            self.sta2_buf = np.zeros([size, 6,npoints], dtype=np.float32)
...
class HLAgent:
    def __init__(self, mode='de', width=128, device=0, use_fast=False, task=1,
        ...
        if self.use_fast:
            ...
            self.buffer = ReplayBuffer(self.mode, self.width, self.width, 5, 20, size=100000)
        print('******************************************\n'
              'Create Agent: \n'
              '     task:', task, ';\n'
              '     mode:', self.mode, ';\n'
        ...
        if self.use_fast:
            ...
            if self.mode == 'pc':
                self.actor = opt_cuda(Actor_pc(),self.device)
                self.target_actor = opt_cuda(Actor_pc(),self.device)
            else:
